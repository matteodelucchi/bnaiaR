...
PLOTPATH <- "~/spaces/work/bnaiaR/inst/extdata/mcmcabn/results/"
DATPATH <- "~/spaces/work/bnaiaR/inst/extdata/mcmcabn/results/raw/"
PLOTWIDTH = 16
PLOTHEIGHT = 9
```
...
if (SAVEPLOTS){
  ...
  dev.print(pdf, file = paste0(PLOTPATH, FILENAME, PLOTNAME, ".pdf"), width = PLOTWIDTH, height = PLOTHEIGHT)
  dev.off()
} else {
  gelman.plot(list.mc.out.thin.score, autoburnin = T)
}
...
if (SAVEPLOTS){
  ...
  dev.print(pdf, file = paste0(PLOTPATH, FILENAME, PLOTNAME, ".pdf"), width = PLOTWIDTH/2, height = PLOTHEIGHT/2)
  dev.off()
...
if (SAVEPLOTS){
  ...
  Rgraphviz::plot(cons.dag.plt.edgestrength)
  # dev.print(svg, filename = paste0(getwd(), PLOTPATH, FILENAME, PLOTNAME, ".svg"), width = PLOTWIDTH/2, height = PLOTHEIGHT/2)
  dev.print(pdf, file = paste0(PLOTPATH, FILENAME, PLOTNAME, ".pdf"), width = PLOTWIDTH/2, height = PLOTHEIGHT/2)
  dev.off()
  } else {
...
basicresulttable <- function(FITABN=fabn.boot.th.mle, SUPPORTDAG=dag.mcmc.boot){
  ...
  possibleLevels <- c("\\.Former", "\\.No", "\\.Current", "\\.No", "\\.Yes", "\\.Low", "\\.Medium", "\\.High", "High", "Low", "Medium", "Acom", "Pcom", "V-B", "A2", "PC", "MCA", "ICA", "Basilar", "Other", "A1 segment ant", "CavICA", "OphtICA")
  support <- c()
  from_names <- c()
  from_levels <- c()
  to_names <- c()
  ...
  for (i in 1:length(cornames)){
    ...
    if (length(tolevels) == 1){
      ...
      tolevels <- stringr::str_split(as.character(na.omit(stringr::str_match(edgename[2], possibleLevels))), "\\.", simplify = T)[1,2] # TODO: Extend this loop case specific matching. as  [1,2] doesn't fit for IA_Locations with multiple childs/parent combinations.
    ...
    } else if ("IA_Location" %in% fromname){
      fromname <- "IA_Location"
