...
png(filename = paste0(getwd(), "/results/", FILENAME, "_maxparentDAGs.png"))
par(mfrow=c(3,3))
parents <- 1
for(i in plt){
  plot(i)
...
TODO: Continue here (run mcmcabn_multinomial again)
...
for (i in 1:nrow(result.table)){
  if(stringr::str_detect(result.table$interpretation[i], "ratio")){
...
if (SAVEPLOTS){
  write.csv(result.table.mle, file = paste0(getwd(), PLOTPATH, FILENAME, "results_table_mle.csv"))
...
TODO: Include support values for multinomial parent nodes
