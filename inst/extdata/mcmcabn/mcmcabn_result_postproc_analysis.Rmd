...
result.table <- result.table.basic %>%
  ...
  mutate(toLevels = case_when(to == "Ruptured_IA" ~ "Yes, No",
                              TRUE ~ toLevels)) %>%
...
result.table.mle %>%
  ...
  cat(., file = paste0(getwd(), PLOTPATH, FILENAME, "results_table_mle_test.tex"))
  # kableExtra::save_kable(paste0(getwd(), PLOTPATH, FILENAME, "results_table_mle_test.pdf"), keep_tex = T)
```
