...
result.table <- result.table[which(!stringr::str_detect(result.table$cornames, "intercept")),]

for (i in 1:nrow(result.table)){
  # Because of logit() scale
  result.table$expcoef[i] <- exp(result.table$coefficient[i])
  if(result.table$interpretation[i] != "correlation"){
    result.table$expinterpret[i] <- "odds ratio"
  } else {
    result.table$expinterpret[i] <- "exp. cor. coef."
  }
  
 if(result.table$expcoef[i]<1 && result.table$interpretation[i] != "correlation"){
    result.table$association[i] <- "negative"
  } else if (result.table$expcoef[i]>1 && result.table$interpretation[i] != "correlation"){
  ...
  # childrenParent <- stringr::str_split(result.table$cornames[i], "\\|")
...
result.table.mle <- result.table %>%
  mutate(coefficient = as.numeric(round(coefficient, 2)),
         expcoef = as.numeric(round(expcoef, 2)),
         SE = as.numeric(round(SE, 2))) %>%
  select(c(from, fromLevels, to, toLevels, expcoef, expinterpret, association, coefficient, SE, interpretation, support, cornames))
...
result.table.mle %>%
  ...
  arrange(from, fromlevels, to, tolevels) %>%
  # Fix wrong levels for rupture status
  mutate(toLevels = case_when(to == "Ruptured_IA" & toLevels != "Yes, No" ~ "Yes, No", 
                              TRUE ~ toLevels)) %>%
  ...
  # Clean column names
  rename("From" = "from",
         "From Levels" = "fromLevels",
         "To" = "to",
         "To Levels" = "toLevels",
         "Coefficient" = "expcoef",
         "Interpretation" = "expinterpret",
  ...
  kableExtra::kbl(format = "latex",
                  label = "tab:expABNmultinom_100k_results_table_mle",
                  booktabs = T,
  ...
  kableExtra::add_header_above(c("Parent" = 2, "Children" = 2, "Association" = 6, " " = 1)) %>%
  kableExtra::landscape()   %>%
  cat(., file = paste0(getwd(), PLOTPATH, FILENAME, "results_table_mle.tex"))
```
