...
# %>% pivot_wider(names_from = c("IAruptured"), values_from = c(centre_geneva, centre_kuopio, centre_nantes, centre_ucl, centre_utrecht))
  ...
  add_row(rowname = "Sex", no = "", yes="", .after = 6) %>%
  add_row(rowname = "Age", no = "", yes="", .after = 9) %>%
  add_row(rowname = "Familial history", no = "", yes="", .after = 18) %>%
  add_row(rowname = "Hypertension", no = "", yes="", .after = 21) %>%
  add_row(rowname = "Smoking", no = "", yes="", .after = 24) %>%
  add_row(rowname = "IA Multiplicity", no = "", yes="", .after = 27) %>%
  add_row(rowname = "IA Location", no = "", yes="", .after = 30) %>%
  add_row(rowname = "IA Size", no = "", yes="", .after = 34) %>%
...
TODO: Think abou coloring the background of the rows according the blacklist graphic.
