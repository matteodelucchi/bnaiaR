...
```{r learn_with_bic_and_threshold, warning=FALSE}
...
```{r}
# cl = makeCluster(6)
cl = makeCluster(10, outfile = path.expand(paste0(getwd(), "/glmm.bic.out")))
```
...
```{r}
mclogit::mblogit(formula = as.formula("study_source ~ IAlocation_group"), 
                 data = data.dbn.additive,
                 random = as.formula("~ 1| study_source"))
```
