...
```{r learn_with_aic_nostudysource, warning=FALSE}
cl = makeCluster(cores, 
                 type = CLTYPE)
...
```{r learn_with_bic_nostudysource, warning=FALSE}
bb = boot.strength(data.dbn.nostudy, 
                   algorithm = "tabu", 
                   R = 500,
                   cluster = cl, 
...
```{r learn_with_aic_per_studysource, warning=FALSE}
cl = parallel::makeCluster(cores/7, 
                           type = CLTYPE)
bb_studies_aic <- list()
bb_studies_bic <- list()
...
```{r print_aic_per_studysource, warning=FALSE}
avg_studies = list()
# print all AIC BNs
...
```{r compare_plot_aic_per_studysource, warning=FALSE}
if (SAVEPLOTS) {
...
```{r compare_plot_bic_per_studysource, warning=FALSE}
if (SAVEPLOTS) {
...
```{r learn_with_aic_studysourceASparent, warning=FALSE}
cl = makeCluster(cores, 
                 type = CLTYPE)
...
```{r learn_with_bic_studysourceASparent, warning=FALSE}
bb = boot.strength(data.dbn.allstudy, 
                   algorithm = "tabu", 
                   R = 500,
                   cluster = cl, 
...
# cl = makeCluster(6)
cl = makeCluster(cores, 
                 outfile = path.expand(paste0(getwd(), "/glmm.bic.out")), 
                 type = CLTYPE)
```
...
```{r SL additive discrete BN w/ mixed effects}
stime <- Sys.time()
# peakRAM::peakRAM(
...
```{r plots additive discrete BN w/ mixed effects}
avg = averaged.network(bb)
...
# cl = makeCluster(6)
cl = makeCluster(cores, 
                 outfile = path.expand(paste0(getwd(), "/glmm.bic.1.7.out")), 
                 type = CLTYPE)
```


```{r SL additive discrete BN w/ mixed effects size-loc restricted}
stime <- Sys.time()
...
```{r plots additive discrete BN w/ mixed effects size-loc restricted}
avg = averaged.network(bb)
