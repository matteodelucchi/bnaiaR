...
```{r}
sf <- surv_fit(Surv(time = age_diag, event = IAruptured, type = "right") ~ gender+positive_famillial_history+hpt_aware+smoking_current_notcurrent+multipleIAs+ IAlocation_group+IAsize_diag_log + (1|study_source), data = surv_dat)
survminer::ggsurvplot(sf, pval = T, combine = T)
```
...
survplt.location <- ggsurvplot(surffit.location, conf.int = TRUE, 
           ...
           palette = scales::hue_pal()(3)[c(2,3,1)], ggtheme = theme_minimal())
if (SAVEPLOTS){
