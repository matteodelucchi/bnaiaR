...
## remove pos. fam. history == "probably"

There are only very few cases with a pos. fam. history == probably wo we remove
them and consider pos. fam. history as dichotome variable.
...
```{r}
gam_dat_woUtrecht <- adbisgc_reg %>%
  filter(study_source != "utrecht") %>%
...
str(gam_dat_woUtrecht, give.attr=F)
```

### Data preparation

```{r}
train <- sample.int(n = nrow(gam_dat_woUtrecht), size = 0.7*nrow(gam_dat_woUtrecht), replace = FALSE)
TrainSet <- gam_dat_woUtrecht[train,]
ValidSet <- gam_dat_woUtrecht[-train,]
```
...
# Starting with a simple model
# covariates left untransformed and uncorrelated random effects for the study_source means
glmer_model_woUtrecht <- lme4::glmer(IAruptured ~ gender+age_diag+positive_famillial_history+hpt_aware+smoking_current_former_no+multipleIAs+ IAlocation_group+IAsize_diag_log + (1|study_source), family = "binomial", data=TrainSet) # random group intercept

# equatiomatic::extract_eq(glmer_model, wrap=T, use_coefs = F, terms_per_line = 3)

print(glmer_model_woUtrecht)
print(summary(glmer_model_woUtrecht))
plot(glmer_model_woUtrecht)
```

```{r}
# remove labels for correct variable names
attributes(glmer_model_woUtrecht@frame$IAruptured)$label <- NULL
attributes(glmer_model_woUtrecht@frame$gender)$label <- NULL
attributes(glmer_model_woUtrecht@frame$positive_famillial_history)$label <- NULL
attributes(glmer_model_woUtrecht@frame$hpt_aware)$label <- NULL
attributes(glmer_model_woUtrecht@frame$smoking_current_former_no)$label <- NULL
attributes(glmer_model_woUtrecht@frame$multipleIAs)$label <- NULL
attributes(glmer_model_woUtrecht@frame$IAlocation_group)$label <- NULL
attributes(glmer_model_woUtrecht@frame$study_source)$label <- NULL
attributes(glmer_model_woUtrecht@frame$age_diag)$label <- NULL
attributes(glmer_model_woUtrecht@frame$IAsize_diag_log)$label <- NULL

gtplt <- glmer_model_woUtrecht %>%
  gtsummary::tbl_regression(intercept = T) %>%
  gtsummary::add_global_p() %>%
  gtsummary::bold_p(t = 0.10) %>%
  gtsummary::bold_labels() %>%
