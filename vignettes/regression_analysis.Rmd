...
SAVESUMMARYPLT <- TRUE
# printorsave_dfsummary(df=dfucl, FILENAME = "dfucl", SAVE = SAVESUMMARYPLT, PLOTPATH = PLOTPATH)
```
...
adbisgc_reg <- adbisgc %>%
  ...
  mutate(positive_famillial_history = fct_relevel(positive_famillial_history, c("no", "yes"))) %>%
...
NAs_per_source$comment[which(NAs_per_source$data_source == "ucl")] <- "0.2% of IA location values are missing"
NAs_per_source$comment[which(NAs_per_source$data_source == "nantes")] <- "0.5% of IA ruptured values are missing."
NAs_per_source$comment[which(NAs_per_source$data_source == "utrecht")] <- "Many missing smoking (11%) and multiplicity (4%) values."
NAs_per_source$comment[which(NAs_per_source$data_source == "utrecht_ruptured")] <- "Many missing smoking (37%) values."
NAs_per_source$comment[which(NAs_per_source$data_source == "geneva_aneuquest")] <- "2.4% of IA locations are missing."
NAs_per_source$comment[which(NAs_per_source$data_source == "geneva_aneux")] <- "2.8% of smoking are missing."
NAs_per_source$comment[which(NAs_per_source$data_source == "tum")] <- "76% of IA size are missing, due to unavailable images from treatment somewhere else."
NAs_per_source$comment[which(NAs_per_source$data_source == "kssg")] <- "2.9% of IA size are missing, due to unavailable images from external sources."

NAs_per_source


# df_bnaiar %>%
#   filter(study_source == "nantes") %>%
#   summarytools::dfSummary()
# 
# adbisgc_reg %>%
#   filter(data_source == "kssg") %>%
#   summarytools::dfSummary()
```
...
printorsave_dfsummary(df=df_bnaiar_compl, FILENAME = "df_bnaiar_compl", SAVE = SAVESUMMARYPLT, PLOTPATH = PLOTPATH)
...
if (SAVEPLOTS){
  ...
  FILE <- "logreg_IArupture"
  FILENAME <- paste0(FILE, ".", PLOTFORMAT)
  
  ggsave(plot = plt.mod, filename = FILENAME, path = PLOTPATH, 
...
gtplt <- glm_model.logreg.all$finalModel %>%
  gtsummary::tbl_regression(intercept = T) %>% 
  gtsummary::add_global_p() %>%
  gtsummary::bold_p(t = 0.10) %>%
  gtsummary::bold_labels() %>%
  gtsummary::italicize_levels()  

if (SAVEPLOTS){
  FILE <- "glm_model.logreg.all"
  FILENAME <- paste0(FILE, ".", PLOTFORMAT)

  gtplt %>%
    gtsummary::as_gt() %>%
    gt::gtsave(filename = FILENAME, path = PLOTPATH)
...
```{r}
# remove labels for correct variable names
attributes(gam_model_3$model$IAruptured)$label <- NULL
attributes(gam_model_3$model$gender)$label <- NULL
attributes(gam_model_3$model$positive_famillial_history)$label <- NULL
attributes(gam_model_3$model$hpt_aware)$label <- NULL
attributes(gam_model_3$model$smoking_current_notcurrent)$label <- NULL
attributes(gam_model_3$model$multipleIAs)$label <- NULL
attributes(gam_model_3$model$IAlocation_group)$label <- NULL
attributes(gam_model_3$model$study_source)$label <- NULL
attributes(gam_model_3$model$age_diag)$label <- NULL
attributes(gam_model_3$model$IAsize_diag_log)$label <- NULL

gtplt <- gam_model_3 %>%
  gtsummary::tbl_regression(intercept = T,
                            tidy_fun = gtsummary::tidy_gam) %>% 
  # gtsummary::add_global_p(anova_fun = function(x){mgcv::anova.gam(x)%>%broom::tidy(.)}) %>%
  gtsummary::add_global_p(anova_fun = broom::tidy) %>%
  gtsummary::bold_p(t = 0.10) %>%
  gtsummary::bold_labels() %>%
  gtsummary::italicize_levels() 
...
```{r}
# remove labels for correct variable names
attributes(gam_model_4$model$IAruptured)$label <- NULL
attributes(gam_model_4$model$gender)$label <- NULL
attributes(gam_model_4$model$positive_famillial_history)$label <- NULL
attributes(gam_model_4$model$hpt_aware)$label <- NULL
attributes(gam_model_4$model$smoking_current_notcurrent)$label <- NULL
attributes(gam_model_4$model$multipleIAs)$label <- NULL
attributes(gam_model_4$model$IAlocation_group)$label <- NULL
attributes(gam_model_4$model$study_source)$label <- NULL
attributes(gam_model_4$model$age_diag)$label <- NULL
attributes(gam_model_4$model$IAsize_diag_log)$label <- NULL
...
```{r}
# remove labels for correct variable names
attributes(gam_model_12$model$IAruptured)$label <- NULL
attributes(gam_model_12$model$gender)$label <- NULL
attributes(gam_model_12$model$positive_famillial_history)$label <- NULL
attributes(gam_model_12$model$hpt_aware)$label <- NULL
attributes(gam_model_12$model$smoking_current_notcurrent)$label <- NULL
attributes(gam_model_12$model$multipleIAs)$label <- NULL
attributes(gam_model_12$model$IAlocation_group)$label <- NULL
attributes(gam_model_12$model$study_source)$label <- NULL
attributes(gam_model_12$model$age_diag)$label <- NULL
attributes(gam_model_12$model$IAsize_diag_log)$label <- NULL
...
```{r}
# remove labels for correct variable names
attributes(glmer_model@frame$IAruptured)$label <- NULL
attributes(glmer_model@frame$gender)$label <- NULL
attributes(glmer_model@frame$positive_famillial_history)$label <- NULL
attributes(glmer_model@frame$hpt_aware)$label <- NULL
attributes(glmer_model@frame$smoking_current_notcurrent)$label <- NULL
attributes(glmer_model@frame$multipleIAs)$label <- NULL
attributes(glmer_model@frame$IAlocation_group)$label <- NULL
attributes(glmer_model@frame$study_source)$label <- NULL
attributes(glmer_model@frame$age_diag)$label <- NULL
attributes(glmer_model@frame$IAsize_diag_log)$label <- NULL
...
# Multivariate Generalized Linear Mixed Effects Model (mixed effects logistic regression) w/o Utrecht (3 levels for smoking)

```{r}
gam_dat_woUtrechtTUM <- adbisgc_reg %>%
  filter(!(study_source %in% c("utrecht", "tum"))) %>% 
  mutate(study_source = factor(study_source)) %>%
  ...
  # remove duplicate variable-versions. Keep contiunuous vars and IA locations by risk.
  select(-c(age_diag_group, IAsize_diag, IAsize_diag_grouped, IAsize_diag_grouped_merged, ID_1, "smoking_current_notcurrent")) %>%
  
  # remove NAs
  na.omit() 

str(gam_dat_woUtrechtTUM, give.attr=F)
```

### Data preparation

```{r}
train <- sample.int(n = nrow(gam_dat_woUtrechtTUM), size = 0.7*nrow(gam_dat_woUtrechtTUM), replace = FALSE)
TrainSet <- gam_dat_woUtrechtTUM[train,]
ValidSet <- gam_dat_woUtrechtTUM[-train,]
```
...
# Starting with a simple model
# covariates left untransformed and uncorrelated random effects for the study_source means
glmer_model_woUtrechtTUM <- lme4::glmer(IAruptured ~ gender+age_diag+positive_famillial_history+hpt_aware+smoking_current_former_no+multipleIAs+ IAlocation_group+IAsize_diag_log + (1|study_source), family = "binomial", data=TrainSet) # random group intercept

# equatiomatic::extract_eq(glmer_model, wrap=T, use_coefs = F, terms_per_line = 3)

print(glmer_model_woUtrechtTUM)
print(summary(glmer_model_woUtrechtTUM))
plot(glmer_model_woUtrechtTUM)
```

```{r}
# remove labels for correct variable names
attributes(glmer_model_woUtrechtTUM@frame$IAruptured)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$gender)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$positive_famillial_history)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$hpt_aware)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$smoking_current_former_no)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$multipleIAs)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$IAlocation_group)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$study_source)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$age_diag)$label <- NULL
attributes(glmer_model_woUtrechtTUM@frame$IAsize_diag_log)$label <- NULL

gtplt <- glmer_model_woUtrechtTUM %>%
  gtsummary::tbl_regression(intercept = T) %>%
  gtsummary::add_global_p() %>%
  gtsummary::bold_p(t = 0.10) %>%
  gtsummary::bold_labels() %>%
