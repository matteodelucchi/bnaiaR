...
```{r}
printorsave_dfsummary(df_bnaiar, FILENAME = "summary_adbisgc_bnaiar", MAXVALS=15, SAVE = TRUE, PLOTPATH = PLOTPATH)
...
printorsave_dfsummary(df_bnaiar_compl, FILENAME = "summary_adbisgc_bnaiar_compl", MAXVALS=15, SAVE = TRUE, PLOTPATH = PLOTPATH)
summarytools::dfSummary(df_bnaiar_compl)
```

```{r}
usethis::use_data(df_bnaiar_compl, overwrite = TRUE)
```

### How many former smoker are among "non-smoker"?
```{r}
smokid <- df_bnaiar_compl %>%
  filter(smoking_current_notcurrent == "not_current") %>%
  select(ID_1) 

adbisgc_reg %>%
  filter(ID_1 %in% smokid$ID_1) %>% 
...
## Experiment 1.1: discrete data with three-level smoking (CFN)

Smoking with three levels (current, former, no smoker) is not for all 'sources' 
available. 

```{r}
bnaiar_vars_smokCFN <- c("ID_1", 
           "study_source", 
           "gender", 
           "age_diag", "age_diag_group", 
           "positive_famillial_history", 
...
df_bnaiar_smokCFN <- adbisgc_reg %>%
  select(all_of(bnaiar_vars_smokCFN))

table(select(df_bnaiar_smokCFN, c("study_source", "smoking_current_former_no")), useNA = "always")
```

It seems as former smoking in utrecht is systematically missing. 
TUM did not collect smoking in that level of detail.
Therefore, we remove Utrecht and TUM completely.
For Geneva, we delete the entries with NAs.

```{r}
df_bnaiar_compl_smokCFN <- df_bnaiar_smokCFN %>% 
  filter(!(study_source %in% c("utrecht", "tum"))) %>% 
  mutate(study_source = factor(study_source)) %>% # remove factor level 'utrecht'
  na.omit() 

summarytools::dfSummary(df_bnaiar_compl_smokCFN)
```

Age: discrete  
Location: grouped by risk of rupture  
Size: discrete and merged (see vignette `data_preprocessing` for details)  
Smoking: Current, former, no

```{r}
exp11_dat <- prep_exp_data(
  dat = df_bnaiar_compl_smokCFN,
  EXPNO = 1.1,
  age = "disc.grouped-multinomial",
  location = "byRisk-multinomial",
  size = "grouped.merged-multinomial",
...
## Exp4.5: 3 level smoking

```{r}
exp45_dat <- prep_exp_data(
  dat = df_bnaiar_compl_smokCFN,
  EXPNO = 45, 
  age = "cont",
  location = "byRisk-multinomial",
  size = "log",
  smoking = "mult",
  studysource.restriction = "unrestricted",
  SAVE = F)
```

```{r}
str(exp45_dat$abndata)
str(exp45_dat$dist)
str(exp45_dat$banned)
str(exp45_dat$bl)
str(exp45_dat$retain)
str(exp45_dat$wl)
```

```{r}
usethis::use_data(exp45_dat, overwrite = TRUE)
```
