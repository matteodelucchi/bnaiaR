...
plt <- adbisgc_tmp %>%
  ...
  t() %>%
  as.data.frame() %>%
  mutate(names = rownames(.)) %T>%
  {data.frame(no_na = .$V1, name = .$names) ->> na_overview} %>%
...
na_overview <- na_overview %>%
  mutate(rel_no_na = round(no_na/nrow(adbisgc_tmp), 3))
...
### Which Study-center collected smoking in what level?
```{r}
adbisgc_tmp %>%
  select(c(source, smoking_current_former_no, smoking_current_notcurrent)) %>%
  group_by(source) %>%
  count(smoking_current_former_no, smoking_current_notcurrent)
```

### Which Study-center collected IA number?
```{r}
adbisgc_tmp %>%
  select(c(source, number_of_IAs, multipleIAs)) %>%
  group_by(source) %>%
  count(number_of_IAs, multipleIAs)
```

### Which Study-center collected IA size?
```{r}
adbisgc_tmp %>%
  group_by(source) %>%
  count(round(IAsize_at_diag))
```
...
```{r}
adbisgc_tmp <- adbisgc_tmp %>%
  mutate(age_diag = round(age_diag, 0)) %>%
  mutate(age_diag_group = wrap_age(age_diag))

table(round(adbisgc_tmp$age_diag, 0), adbisgc_tmp$age_diag_group)
```
...
plt.age5y <- adbisgc_tmp %>%
  ...
  labs(title= "Case-distribution of grouped Age at time of Diagnosis",
          subtitle = "Grouped in bins of 5y for patients between 40y and 65y.",
          caption = stringr::str_wrap("Age at time of diagnosis of IA grouped in A=0-39, B=40-44, C=45-49, D=50-54, E=55-59, F=60-64, G=65-94"),
       y="Number of records",
       x="Age Group")+
  theme(plot.caption = element_text(hjust = 0.5),
        plot.caption.position = "plot")
...
adbisgc_tmp <- adbisgc_tmp %>%
  ...
  mutate(IAlocation_group = factor(IAlocation_group, labels = c("low", "medium", "high"))) # 1: low; 2: medium, 3: high
...
```{r}
summarytools::label(adbisgc_tmp$age_diag_group) <- "age_diag grouped in A=0-39, B=40-44, C=45-49, D=50-54, E=55-59, F=60-64, G=65-94 years."
summarytools::label(adbisgc_tmp$smoking_current_notcurrent) <- "Wrapper of three level smoking, combining former and non-smoker as 'not_current'."
summarytools::label(adbisgc_tmp$multipleIAs) <- "Detection of multiple IAs (binary). Wrapper of number of IAs if number of IAs is provided."
...
summarytools::label(adbisgc_tmp$IAsize_diag) <- "Maximum diameter of IA dome at the time of diagnosis."
summarytools::label(adbisgc_tmp$IAsize_diag_log) <- "Log of 'IAsize_diag'."
...
summarytools::label(adbisgc_tmp$mRS_afterSAH) <- "Modified ranking scale at 1y after ruptured IA or corresponding to maxFUtime (ambiguous)."
summarytools::label(adbisgc_tmp$gos_afterSAH) <- "Glasgow outcome scale at 1y after ruptured IA or corresponding to maxFUtime (ambiguous)."
summarytools::label(adbisgc_tmp$IAruptured) <- "IA rupture status with respect to the time of diagnosis."
...
adbisgc_tmp <- adbisgc_tmp %>% 
  ...
  filter((age_diag >= 10)&(age_diag <=100)|is.na(age_diag)) %>% 
  pipe_message("filter IA size outliers.") %>%
  filter((IAsize_diag_log>=0)&(IAsize_diag_log<=4)|is.na(IAsize_diag_log)) %>%
  pipe_message("Filter probable familial history.") %>%
  filter((positive_famillial_history != "probably")|is.na(positive_famillial_history)) %>%
  mutate(positive_famillial_history = factor(positive_famillial_history))

summarytools::label(adbisgc_tmp$positive_famillial_history) <- "Familiar incidence known (yes/no). 'yes' if aware of one or more 1st degree relative(s) with intracranial aneurysm. 'NA' if familiar history is unknown. 'no' otherwise."
...
saveRDS(adbisgc,
        file = "/home/matteo/ZHAW/ExplorDataISGC/2212_adbisgc_data/2212_adbisgc.rds")
write.csv(adbisgc, 
          file = "/home/matteo/ZHAW/ExplorDataISGC/2212_adbisgc_data/2212_adbisgc.csv")
saveRDS(adbisgc_ohe,
        file = "/home/matteo/ZHAW/ExplorDataISGC/2212_adbisgc_data/2212_adbisgc_ohe.rds")
write.csv(adbisgc_ohe, 
          file = "/home/matteo/ZHAW/ExplorDataISGC/2212_adbisgc_data/2212_adbisgc_ohe.csv")
```
