...
if (SAVEPLOTS){
  ...
  Rgraphviz::renderGraph(cons.dag.plt.edgestrength)
  if(PLOTFORMAT == "svg"){
    dev.print(svg, filename = paste0(FILENAMEbase, FILENAME, filenamesuffix, PLOTNAME, ".", PLOTFORMAT), width = PLOTWIDTH, height = PLOTHEIGHT)
    dev.off()
  } else if(PLOTFORMAT == "pdf"){
...
fabn.boot.th.mle <-fitAbn(dag = dag.mcmc.boot.th,
              ...
              compute.fixed = T,
              create.graph = T)
print(fabn.boot.th.mle)
```

```{r}
out.mle <- unlist(fabn.boot.th.mle$coef, use.names = T) # TODO: keep the names. Unlist manually...
...
if (SAVEPLOTS){
  PLOTNAME <- "_consensus_dag_edgestrength"
  Rgraphviz::renderGraph(cons.dag.plt.edgestrength, attrs=attrs)
  if(PLOTFORMAT == "svg"){
    dev.print(svg, filename = paste0(FILENAMEbase, FILENAME, filenamesuffix, PLOTNAME, ".", PLOTFORMAT), width = PLOTWIDTH, height = PLOTHEIGHT)
    dev.off()
  } else if(PLOTFORMAT == "pdf"){
...
e1 = local({load(paste0(FILENAMEbase, FILENAME, filenamesuffix, "_intermediate.RData")); environment()})
tools:::makeLazyLoadDB(e1, paste0(FILENAMEbase, FILENAME, filenamesuffix, "_intermediate"))
lazyLoad(paste0(FILENAMEbase, FILENAME, filenamesuffix, "_intermediate"))

e2 = local({load(paste0(FILENAMEbase, FILENAME, filenamesuffix, "_paramBootAbnConsDAG.RData")); environment()})
consDAGlist <- get("consDAGlist", envir = e2)
consfit <- get("consfit", envir = e2)
dags <- get("dags", envir = e2) # WARNING: This is huge (34.1GB)!!!
```
...
```{r}
cons.dag.stren <- consDAGlist$strenDAG
cons.dag.threshold <- consDAGlist$arcStrengthThreshold

# relative arc strength
plot(ecdf(cons.dag.stren))
abline(v = cons.dag.threshold, lty=2)
abline(v=0.5)

# # absolute arc strength
# plot(ecdf(consDAGlist$strenDAG * length(dags)))
...
edgestren <- round(consDAGlist$strenDAG, 2)
edgestren[edgestren<=THRESHOLD]<-0
...
```{r}
(df_cv4_5 <- cv_ROCAUC(data = df, node = "IAruptured", parents = names(consfit$abnDag$dag["IAruptured",][which(consfit$abnDag$dag["IAruptured",] == 1)]))) 

if (SAVEPLOTS){
  PLOTNAME <- "_cvROCAUC"
  saveRDS(object = df_cv4_5, file = paste0(FILENAMEbase, FILENAME, filenamesuffix, PLOTNAME, ".rds"))
}
```
