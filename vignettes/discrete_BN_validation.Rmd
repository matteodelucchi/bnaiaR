...
PLOTPATH <- paste0(Sys.getenv("PLOTPATH"))
...
if (SAVEPLOTS){
  PLOTSUFFIX <- "_cvROCAUC"
  saveRDS(object = dmeBN_eval, file = paste0(PLOTPATH, "/", PLOTNAME, PLOTSUFFIX, ".rds"))
}
```
...
if (SAVEPLOTS){
  PLOTSUFFIX <- "_cvROCAUC"
  saveRDS(object = dmeBNsizelocrestricted_eval, file = paste0(PLOTPATH, "/", PLOTNAME, PLOTSUFFIX, ".rds"))
}
```
...
```{r}
cv_4.5 <- readRDS(file = paste0(PLOTPATH, "/expABNmultinomial4.5_cvROCAUC.rds")) # generated in 'continuous_ABN_SL_postprocessing.Rmd'
rocdf_4.5 <- cv_4.5$rocdf
rocdf_4.5$type <- "additive mixed-efffects"
  
rocdf$type = "discrete no study source"

rocdf_me <- dmeBN_eval$rocdf
rocdf_me$type = "discrete mixed-effects"

rocdf_meSizeLocRest <- dmeBNsizelocrestricted_eval$rocdf
rocdf_meSizeLocRest$type <- "discrete mixed-effects Size -> Loc Restricted"

rocplot_all <- rbind(rocdf_4.5, rocdf, rocdf_me, rocdf_meSizeLocRest) %>%
...
```{r}
dcadf_4.5 <- cv_4.5$dca
dcadf_4.5$type <- "additive mixed-efffects"
  
# rocdf$type = "discrete no study source"

dcadf_me <- dmeBN_eval$dca
dcadf_me$type = "discrete mixed-effects"

dcadf_meSizeLocRest <- dmeBNsizelocrestricted_eval$dca
dcadf_meSizeLocRest$type <- "discrete mixed-effects Size -> Loc Restricted"

dcaplt_all <- rbind(dcadf_4.5, dcadf_me, dcadf_meSizeLocRest) %>%
  ggplot(aes(x=probthr, y=value, linetype = name, color= type)) +
  # geom_point() +
  geom_line() +
