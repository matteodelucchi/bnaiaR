...
summarytools::dfSummary(df_bnaiar_compl, max.distinct.values = 15)
```
...
```{r IA size mm overlay unrupture}
df.size <- adbisgc_reg %>% 
  # Remove IA size outliers
  filter(IAsize_diag_log < 4) %>%
  # keep only ruptured IAs
  filter(IAruptured == "no") %>%
  select(c(IAsize_diag, IAsize_diag_grouped_merged)) %>%
  arrange(IAsize_diag)
...
# small geom_text size
plt.overlay.size.mm.unrupt <- df.size.cont %>%
  ggplot() +
...
# large geom_text size
plt.overlay.size.mm.large.unrupt <- df.size.cont %>%
  ggplot() +
  ...
  theme(
    ...
    title = element_text(size = 30),
          ...
          legend.text = element_text(size = 28),
          legend.title = element_text(size = 30))
plt.overlay.size.mm.large.unrupt

if (SAVEPLOTS) {
  ggsave(
    plot = plt.overlay.size.mm.large.unrupt,
    filename = "IAsize_overlay_mm_unrupt.png",
    path = PLOTPATH,
    dpi = 600
  )
} else {
...
```{r IA size mm overlay rupture}
df.size <- adbisgc_reg %>% 
  # Remove IA size outliers
  filter(IAsize_diag_log < 4) %>%
  # keep only ruptured IAs
  filter(IAruptured == "yes") %>%
  select(c(IAsize_diag, IAsize_diag_grouped_merged)) %>%
  arrange(IAsize_diag)
...
# small geom_text size
plt.overlay.size.mm.rupt <- df.size.cont %>%
  ggplot() +
...
# large geom_text size
plt.overlay.size.mm.large.rupt <- df.size.cont %>%
  ggplot() +
  ...
  theme(
    ...
    title = element_text(size = 30),
          ...
          legend.text = element_text(size = 28),
          legend.title = element_text(size = 30))
plt.overlay.size.mm.large.rupt

if (SAVEPLOTS) {
  ggsave(
    plot = plt.overlay.size.mm.large.rupt,
    filename = "IAsize_overlay_mm_rupt.png",
    path = PLOTPATH,
    dpi = 600
  )
} else {
...
```{r Ruptured IA colored by size group}
pltRuptSize_raw <- df_bnaiar_compl %>%
  group_by(IAruptured, IAsize_diag_grouped_merged) %>%
  summarise(nrupt = n()) %>%
  ungroup() %>%
  
  ggplot() +
  aes(x = IAruptured, y=nrupt, fill=IAsize_diag_grouped_merged)  +
  geom_col() +
  # scale_fill_manual(values=c("no"=colorblind_pal()(8)[2], "yes"= colorblind_pal()(8)[4]))
  scale_fill_manual(values=c("A"=colorblind_pal()(8)[2], "B"= colorblind_pal()(8)[4], "C"= colorblind_pal()(8)[7]))

pltRuptSize <- pltRuptSize_raw+
  labs(title = "IA size",
       fill = "IA size grouped",
       x = "IA ruptured",
       y = "Number of records")+
  theme(axis.text.x.bottom = element_blank())+
  theme_tufte()
...
if (SAVEPLOTS) {
  ggsave(
    plot = pltRuptSize,
    filename = "IArupture_bySize_mm.png",
    ...
    ggsave(
    plot = pltRuptSize,
    filename = "IArupture_bySize_mm.pdf",
    width = PLOTWIDTH, height = PLOTHEIGHT,
    path = PLOTPATH
  )
} else {
...
pltSizeRupt_raw <- df_bnaiar_compl %>%
  group_by(IAsize_diag_grouped_merged, IAruptured) %>%
  summarise(nsize = n()) %>%
  ungroup() %>%
  
  ggplot() +
  aes(x = IAsize_diag_grouped_merged, y=nsize, fill=IAruptured)  +
  geom_col() +
  scale_fill_manual(values=c("yes"=colorblind_pal()(8)[2], "no"= colorblind_pal()(8)[4]))

pltSizeRupt <- pltSizeRupt_raw+
  labs(title = "IA size",
       fill = "IA ruptured",
       x = "IA size grouped",
       y = "Number of records")+
...
```{r Ruptured IA colored by size group and overlayed by size in mm}
### remove attributes
df <- data.frame(df_bnaiar_compl) # remove attributes
df[] <- lapply(df_bnaiar_compl, c) # remove attributes

df <- df %>%
  # select(IAsize_diag_log, IAsize_diag_grouped_merged, IAruptured) %>%
  select(IAsize_diag, IAsize_diag_grouped_merged, IAruptured) 

# show_col(colorblind_pal()(8))

plt.overlay.size.mm.large.rupt <- df %>%
  ggplot() +
  geom_histogram(aes(x=IAsize_diag, y=..density.., fill=IAruptured, alpha = 0.5, color = "black"), breaks = c(0,7,12,54))+
  geom_density(aes(x=IAsize_diag, color=IAruptured)) +
  # geom_histogram(aes(x=IAsize_diag, y=..density.., fill=IAruptured)) +
  
  scale_x_continuous(breaks = round(c(seq(0, 60, 5)), 2)) +
  # scale_x_continuous(breaks = c(0, 5, 7, 10, 12, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60)) +
  ...
  scale_fill_manual(values=c("yes"=colorblind_pal()(8)[2], "no"= colorblind_pal()(8)[4]))+
  scale_color_manual(values=c("yes"=colorblind_pal()(8)[2], "no"= colorblind_pal()(8)[4]))+
...
plt.overlay.size.mm.small.rupt <- df %>%
  ggplot() +
  geom_histogram(aes(x=IAsize_diag, y=..density.., fill=IAruptured, alpha = 0.5, color = "black"), breaks = c(0,7,12,54))+
  geom_density(aes(x=IAsize_diag, color=IAruptured)) +
  # geom_histogram(aes(x=IAsize_diag, y=..density.., fill=IAruptured)) +
  ...
  geom_text(label = "A", family = "serif", fontface = "plain",
            x=3.5,
            y=0.195)+
  geom_text(label = "B",  family = "serif", fontface = "plain",
            x=9.5,
            y=0.106)+
  geom_text(label = "C",  family = "serif", fontface = "plain",
            x=33.5,
            y=0.01)+
...
plt.overlay.size.mm.small.rupt

if (SAVEPLOTS) {
  ggsave(
    plot = plt.overlay.size.mm.large.rupt,
    filename = "IAsize_overlay_mm_byrupt.png",
    width = PLOTWIDTH, height = PLOTHEIGHT,
    path = PLOTPATH,
    dpi = 600
  )
    ggsave(
    plot = plt.overlay.size.mm.large.rupt,
    filename = "IAsize_overlay_mm_byrupt.pdf",
    width = PLOTWIDTH, height = PLOTHEIGHT,
    path = PLOTPATH
  )
} else {
...
```
Caption:
Kernel density estimates for IA size at diagnosis stratified by rupture status
The continuous variable IA size at diagnosis was discretised using clinical conventions: IA size group A ≤ 7mm, 7mm < B  ≤ 12mm, C > 12mm.
